		...
		var setGridWidth = function() {
			...
			if (gridWidth != 0) {
				dataGrid.jqGrid("setGridWidth", gridWidth - options.autoGridWidthFix,
						(options.shrinkToFit && $(window).width() > 500))
			}
		};
		setGridWidth();
		setTimeout(function() {
...
$.fn.dataGrid = function(c, j, i, h, f, e) {
	var a;
	var b = this.each(function() {
		var m = $(this);
		var l = m.data("dataGrid");
		...
		if (typeof c === "string" && typeof l[c] === "function") {
			if (j instanceof Array) {
				a = l[c].apply(l, j, i, h, f, e)
			} else {
				a = l[c](j, i, h, f, e)
			}
		}
	...
	if (a === undefined) {
		var g = $.jgrid.getMethod(c);
		if (g) {
			var d = $.makeArray(arguments).slice(1);
			a = g.apply(this, d)
		}
	}
...
function page(b, a, d) {
	try {
		if (d && d != "") {
			if (dataGrids != undefined && dataGrids[d].refresh != undefined) {
				dataGrids[d].refresh(b, a)
			} else {
				$("#" + d).dataGrid("refresh", b, a)
			}
		} else {
			if (dataGrid != undefined && dataGrid.refresh != undefined) {
				dataGrid.refresh(b, a)
			} else {
			...
			.extend({
				setComplexHeaders : function(b) {
					...
					return this
							.each(function() {
								...
								var K = this, p = l.defaultStyle === undefined ? true : l.defaultStyle, G, E, n, J, k, d, F, x, M, u, c, C, r = 0, H = 0, A = false, B, v, w, f, y = K.p.colModel, D = y.length, t = K.grid.headers, h = a(
										"table.ui-jqgrid-htable", K.grid.hDiv), m = h.children("thead")
										.children("tr.ui-jqgrid-labels:last").addClass("jqg-second-row-header"), e = h.children("thead"), s, q, o = h
				...
				createFrozenHtable : function(e, g) {
					var P = this[0], I = P.p.complexHeader, m = I.complexHeaders, q = m.defaultStyle, k = m.threeLevel, j = m.twoLevel, H = P.p.id, A = P.p.colModel, F = e + 1, t = P.grid.headers, r = 0, L = 0, C = false, K, G, z, d, J, y, B, R, l, D, u, b, E, v, w, h, c = P.grid.fhDiv
							.height(), M = a("<table class='ui-jqgrid-htable' style='width:1px;height:" + c
							+ "px' role='grid' aria-labelledby='gbox_" + H + "' cellspacing='0' cellpadding='0' border='0'></table>"), f = a("<thead></thead>"), p = a(
							"<tr>", {
								role : "row",
								"aria-hidden" : "true"
