    ...
    me.commands[thePlugins] = {
        execCommand:function () {
            var dialog = new UE.ui.Dialog({
                ...
                editor:this,
                title: '下拉菜单',
                cssRules:"width:600px;height:465px;",
                buttons:[
					{
						className:'edui-okbutton',
						label:'加入标签库',
 ...
 * 隐藏框
 */
UE.plugins['hidden'] = function () {
	var me = this,thePlugins = 'hidden';
	me.commands[thePlugins] = {
		execCommand:function () {
			var dialog = new UE.ui.Dialog({
				iframeUrl:this.options.UEDITOR_HOME_URL + UE.leipiFormDesignUrl+'/text.html',
	...
	me.addListener( 'mouseover', function( t, evt ) {
		...
		if ( /input/ig.test( el.tagName ) && leipiPlugins==thePlugins) {
			var html = popup.formatHtml(
				'<nobr>隐藏框: <span onclick=$$._edittext() class="edui-clickable">编辑</span>&nbsp;&nbsp;<span onclick=$$._delete() class="edui-clickable">删除</span></nobr>' );
			if ( html ) {
				popup.getDom( 'content' ).innerHTML = html;
				popup.anchorEl = el;
				popup.showAnchor( popup.anchorEl );
